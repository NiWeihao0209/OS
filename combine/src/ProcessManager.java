...
public class ProcessManager implements Runnable {
    ...
    public void release(int pid){
        ...
        for(int waitpid = 0; waitpid < waitingQueue.size(); waitpid++){
            if(pcbList.get(waitpid).status != "wating(Printer)"){
                //根据下一条指令计算出预计需要使用设备的时间
                //调用设备提供的方法插入到设备中
                //*************
                //修改改pcb的status
                pcbList.get(waitpid).status = "waiting(Printer)";
            }
        }
    }
    ...
    public void killProcess(int pid) {
        ...
        for (int i = 0; i < pcbList.size(); i++) {
            ...
            if (pcb.pid == pid) {
                ...
                pcbList.get(pid).status = "terminated";

                // 根据之前的状态释放对应的资源
                if(pcb.status == "ready"){
                    readyQueue.get(pcbList.get(pid).priority).remove(pid);
